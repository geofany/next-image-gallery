...
function getPagesNumber(url: string) {
  const { searchParams } = new URL(url)
  return searchParams.get('page')
}

export default function getPrevNextPages(images: ImagesResults) {
  let nextPage = images?.next_page ? getPagesNumber(images.next_page) : null
  const prevPage = images?.prev_page ? getPagesNumber(images.prev_page) : null
